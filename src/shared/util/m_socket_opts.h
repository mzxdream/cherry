...
public:
    ...
    static MError Connect(const std::string &ip, unsigned port);
    static std::pair<int, MError> Send(const char *p_buf, int len);
    static std::pair<int, MError> Recv(void *p_buf, int len);
    static MError SetBlock(bool block);
    static MError SetReUseAddr(bool re_use);
};
